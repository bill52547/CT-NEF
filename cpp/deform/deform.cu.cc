             ...
             float *img1)
{
    ...
    int zi2 = zi + 1;
    if (xi <= 0 || xi2 >= nx) {return;}
    if (yi <= 0 || yi2 >= ny) {return;}
    if (zi <= 0 || zi2 >= nz) {return;}
    float wx2 = ix + mx[id] - xi; float wx = 1.0f - wx2;
    float wy2 = iy + my[id] - yi; float wy = 1.0f - wy2;
    float wz2 = iz + mz[id] - zi; float wz = 1.0f - wz2;
            ...
            float *img1)
{
    ...
    if (yi < 0 || yi2 >= ny) {return;}
    if (zi < 0 || zi2 >= nz) {return;}
    float wx2 = ix + mx - xi; float wx = 1.0f - wx2;
    float wy2 = iy + my - yi; float wy = 1.0f - wy2;
    float wz2 = iz + mz - zi; float wz = 1.0f - wz2;
    img1[id] =  img[xi + yi * nx + zi * nx * ny] * wx * wy * wz +
                img[xi + yi * nx + zi2 * nx * ny] * wx * wy * wz2 +
                img[xi + yi2 * nx + zi * nx * ny] * wx * wy2 * wz +
                img[xi + yi2 * nx + zi2 * nx * ny] * wx * wy2 * wz2 +
            ...
            float *img1)
{
    ...
    float my = (iy + 0.5f - ny / 2) * sin_phi + (iy + 0.5f - ny / 2) * cos_phi + ny / 2;
    int xi = (int)floor(mx);
    int yi = (int)floor(my);
    int xi2 = xi + 1;
    int yi2 = yi + 1;
    if (xi <= 0 || xi2 >= nx) {return;}
    if (yi <= 0 || yi2 >= ny) {return;}
    float wx2 = mx - xi; float wx = 1.0f - wx2;
    float wy2 = my - yi; float wy = 1.0f - wy2;
