...
extern "C" void backproject(float *image,float *proj_values, 
...
__global__ void kernel_backprojection(float *image, const float *vproj,
        ...
        for (int ix = 0; ix < nx; ix++){
            ...
            for (int iy = MAX(0, Yi1); iy <= MIN(ny - 1, Yi2); iy++)
            {
                ...
                for (int iz = MAX(0, Zi1); iz <= MIN(nz - 1, Zi2); iz++)
                {
                    ...
                    atomicAdd(image + ix + iy * nx + iz * nx * ny, vproj[id] * wy * wz);
                }                
            }        
        }
    }
        ...
        for (int iy = 0; iy < ny; iy++){
            ...
            for (int ix = MAX(0, Xi1); ix <= MIN(nx - 1, Xi2); ix++)
            {
                ...
                for (int iz = MAX(0, Zi1); iz <= MIN(nz - 1, Zi2); iz++)
                {
                    ...
                    atomicAdd(image + ix + iy * nx + iz * nx * ny, vproj[id] * wx * wz);
                }                
            }        
        }            
    }
...
void backproject(float *image,float *proj_values, 
             float *angles, float *off_a, float *off_b, 
             float *timestamps,
             int nx, int ny, int nz,
             float dx, float dy, float dz,
             ...
             float da, int na,
             float db, int nb,
             int num_angles, int num_cate_timestamps)
{
    float *d_proj_values, *d_image, *d_angles, *d_off_a, *d_off_b, *d_timestamps;
    ...
    cudaMalloc((void**)&d_timestamps, size_angles * num_cate_timestamps);
    cudaMemcpy(d_timestamps, timestamps, size_angles * num_cate_timestamps, cudaMemcpyHostToDevice);
    ...
    kernel_backprojection<<<shapeSize, blockSize>>>(d_image, 
                                                d_proj_values, 
                                                ...
                                                SID, SAD, 
                                                da, na, db, nb, 
                                                num_angles, num_cate_timestamps);
    cudaMemcpy(image, d_image, size_image, cudaMemcpyDeviceToHost);
