...
__global__ void kernel_generate_back(const int mode,
                            ...
                            const int *row, int *col, float *data)
{
    ...
    const dim3 shapeSize((nx + GRIDDIM_X - 1) / GRIDDIM_X, (ny + GRIDDIM_Y - 1) / GRIDDIM_Y, (nx + GRIDDIM_Z - 1) / GRIDDIM_Z);
    const dim3 blockSize(GRIDDIM_X, GRIDDIM_Y, GRIDDIM_Z);
    kernel_generate_back<<<shapeSize, blockSize>>>(0,
                                                   SD, SO,
                                                   nx, ny, nz,
                                                   da, ai, na,
                                                   db, bi, nb,
                            ...
                            const int *row, int *col, float *data)
{
    ...
    const dim3 shapeSize((nx + GRIDDIM_X - 1) / GRIDDIM_X, (ny + GRIDDIM_Y - 1) / GRIDDIM_Y, (nx + GRIDDIM_Z - 1) / GRIDDIM_Z);
    const dim3 blockSize(GRIDDIM_X, GRIDDIM_Y, GRIDDIM_Z);
    kernel_generate_back<<<shapeSize, blockSize>>>(1,
                                                   SD, SO,
                                                   nx, ny, nz,
                                                   da, ai, na,
                                                   db, bi, nb,
