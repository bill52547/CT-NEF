                                  ...
                                  float *vproj)
{
    ...
    if (mode == 0)
    {
        ...
        z20 = (ib + 0.5) * db + bi - nb * db / 2 + offsets[0];
    }
    else
    {
    ...
    y21 = y2 - y1;
    z21 = z2 - z1;
    float l = (float)sqrtf(x21 * x21 + y21 * y21 + z21 * z21);
    // y - z plane, where ABS(x21) > ABS(y21)
    if (ABS(x21) > ABS(y21))
    {
        float cos_alpha = ABS(x21) / sqrtf(x21 * x21 + y21 * y21);
        float cos_gamma = ABS(x21) / sqrtf(x21 * x21 + z21 * z21);
        ...
        for (int ix = 0; ix < nx; ix++)
        {
            ...
            for (int iy = MAX(0, Yi1); iy <= MIN(ny - 1, Yi2); iy++)
            {
                ...
                for (int iz = MAX(0, Zi1); iz <= MIN(nz - 1, Zi2); iz++)
                {
                    ...
                    wz /= (zi2 - zi1);
                    vproj[id] += image[ix + iy * nx + iz * nx * ny] * wy * wz;// / cos_alpha / cos_gamma;
                }
            }
        }
    }
    ...
    {
        float cos_alpha = ABS(y21) / sqrtf(x21 * x21 + y21 * y21);
        float cos_gamma = ABS(y21) / sqrtf(y21 * y21 + z21 * z21);
        ...
        for (int iy = 0; iy < ny; iy++)
        {
            ...
            for (int ix = MAX(0, Xi1); ix <= MIN(nx - 1, Xi2); ix++)
            {
                ...
                for (int iz = MAX(0, Zi1); iz <= MIN(nz - 1, Zi2); iz++)
                {
                    wz = MIN(iz + 1.0f, zi2) - MAX(iz + 0.0f, zi1);
                    wz /= (zi2 - zi1);
                    vproj[id] += image[ix + iy * nx + iz * nx * ny] * wx * wz;// / cos_alpha / cos_gamma;
                }
            }
        }
    }
